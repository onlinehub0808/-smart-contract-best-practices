...
- [**已知的攻击手段**](#known-attacks)
  ...
  - [***上溢/下溢***](#integer-overflow-and-underflow)
- [**工程技术**](#eng-techniques)
- [**参考文献**](#bibliography)

这篇文档旨在为Solidity开发人员提供一些智能合约的**security**准则。当然也包括智能合约的**security理念、bug赏金计划指南、文档例程以及工具。**
...
- **对可能的错误有所准备**。任何有意义的智能合约或多或少都存在错误。因此你的代码必须能够正确的处理出现的bug和漏洞。始终保证以下规则：
  - 当智能合约出现错误时使智能合约挂起（“断路开关”）
  - 防范可能的资金风险（限制（转账）速率、最大（转账）额度）
  - 有效的途径来进行bug修复和功能提升

- [**谨慎发布智能合约**](#contract-rollout)。 尽量在正式发布智能合约之前修复bug。
  - 对智能合约进行彻底的测试，并在任何新的攻击手法被发现后及时的测试
  - 从发布到alpha testnet开始便提供[bug赏金计划](#bounties) 
  - 阶段性发布，每个阶段都提供足够的测试

- **保持智能合约的简洁**。复杂只会增加出错的风险。
  - 确保智能合约逻辑简洁
  - 确保合约和函数模块化
...
从软件工程的角度出发一个理想的智能合约首先需要模块化，能够重用代码而不是重复编写，并且支持组件升级。从安全架构的角度出发一个理想的智能合约需要尽可能做到共享，特别是在智能合约这种复杂的应用场景。

然而，这里有几个重要的例外，它们在安全和软件工程最佳实践里可能没有被衡量。当中每一条，都需要针对智能合约系统的特点找到最优的组合方式来达到平衡。
...
在多个场景中，包括这个，都会强调延展性比如：可终止，可升级或可更改的特性，以上在延展性和安全之间这是个*基本权衡*。

延展性和复杂性会增加潜在的攻击面。在智能合约这种只在特定的时间段内提供有限的功能的使用场景下，简单性比复杂性显得更加高效，比如governance-fee、finite-time-frame、token-sale的智能合约系统。

#### 庞大 vs 模块化

一个庞大的独立的智能合约把所有的可识别和可读取的信息都放到本地。尽管很少有智能合约真的做到了大体量，但在是否应该将数据和流程都放到本地这个问题上仍然存在争议--比如，优化代码review效率。

和在这里讨论的其他权衡点一样，智能合约的最佳安全实践从软件工程中提倡简单短生命周期转移到了提倡更加复杂的永久性。

#### 重复 vs 可重用

从软件工程角度看，智能合约系统希望在合理的情况下最大程度地实现重用。 在Solidity中重用合约代码有很多方法。 使用**你拥有的**以前部署的经过验证的智能合约是实现代码重用的最安全的方式。

在以前所拥有已部署智能合约不可用时重复还是很需要的。 现在[Live Libs](https://github.com/ConsenSys/live-libs) 和[Zeppelin Solidity](https://github.com/OpenZeppelin/zeppelin-solidity) 正寻求提供安全的智能合约组件使其能够被重用而不需要每次都重新编写。任何合约安全性分析都必须包括以前没有建立与目标智能合同系统中处于风险中的资金相称的信任级别的重用代码。
...
除了关注核心开发成员，参与到各个区块链安全社区更加重要，因为安全漏洞的披露或研究将通过各方进行。
...
攻击者可以只调用`getLock()`，然后就不在调用 `releaseLock()`。如果他们真这样做，那么这个合约将会被永久锁住，任何接下来的操作都不会发生了。如果你使用mutexs来避免竞态，那么一定要确保没有地方能够打断锁的进程或绝不释放锁。（这里还有一个潜在的威胁，比如死锁和活锁（*译者注：livelocks，我第一次听说*）。在你决定使用锁之前最好大量阅读相关文献（*译者注：这是真的，传统的在多线程环境下对锁的使用一直是个容易犯错的地方*））
...
以上是涉及攻击者在**单个交易**内执行恶意代码产生竞态的示例。接下来演示在区块链上存在的竞态：（同一个block内的）交易顺序很容易受到操纵。
...
对于下溢同样的道理。如果一个uiint别改变后小于0，那么将会导致它下溢并且被设置成为最大值（2^256）。
...
```
当它试图退款给旧的leader时，如果退款失败则会抛出异常。这意味着，恶意投标人可以成为领导者，同时确保对其地址的任何退款**始终**失败。这样就可以阻止任何人调用“bid()”函数，使自己永远保持领先。建议向之前所说的那样建立[基于pull的支付系统](https://github.com/ConsenSys/smart-contract-best-practices/#favor-pull-over-push-payments) 。
...
注意：当你向你的系统添加新的组件时总是伴随着风险的。一个不良设计本身会成为漏洞-一些精心设计的组件在交互过程中同样会出现漏洞。仔细考虑你在合约里使用的每一项技术，以及如何将它们整合共同创建一个健壮的系统。

### 升级有问题的合约

如果代码中发现了错误或者需要对某些部分做改进都需要更改代码。在以太坊上发现一个错误是没有好处的，而且没有办法处理它们。
...
当需要多方参与决定升级代码的方式也是至关重要的。根据你的合约，升级代码可能会需要通过单个或多个受信任方参与投票决定。如果这个过程会持续很长时间，你就必须要考虑是否要换成一种更加高效的方式以防止遭受到攻击，例如[紧急停止或断路器](https://github.com/ConsenSys/smart-contract-best-practices/#circuit-breakers-pause-contract-functionality)。
...
2、在你替换了合约后你需要仔细考虑如何处理合约数据

替代方法是设计一个用来转发调用请求和数据到最新版的合约：
...
- 规范，图表，状态机，模型和其他文档，帮助审核人员和社区了解系统打算做什么。
- 许多bug从规范中就能找到，而且它们的修复成本最低。
- 发布计划所涉及到的参考[这里](https://github.com/ConsenSys/smart-contract-best-practices#contract-rollout)列出的详细信息和完成日期。
...
- 所有已知的错误/限制
- 潜在的攻击和解决办法
- 潜在的利益冲突（例如，纳入自己的腰包，像Slock.it与DAO一样）
