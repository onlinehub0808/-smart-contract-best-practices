...
If a balance reaches the maximum uint value (2^256) it will circle back to zero. This checks for that condition. This may or may not be relevant, depending on the implementation. Think about whether or not the uint value has an opportunity to approach such a large number. Think about how the a uint variable changes state, and who has authority to make such changes. In token applications, any user can call functions which update this uint value. If only an admin has access to change the variables state, you might be safe. If a user only can increment by 1 at a time, you are probably also safe because there is no feasible way to reach this limit.
...
Be careful with the smaller data-types like uint8, 16, 24...etc. They can even more easily hit their maximum value.
